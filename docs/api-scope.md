...
### Queries
- `folderTree: [Folder!]!`: Get the full hierarchical tree structure of folders allowed for the user.
...
- `metadataDetail(id: ID!): Metadata`: Get the full details of a specific metadata item.
...
### Mutations
- `createFolder(name: String!, parentId: ID): Folder!`: Create a new folder under a parent (or root if parentId is null).
- `updateFolder(id: ID!, name: String!): Folder!`: Update a folder's name.
...
- `createMetadata(folderId: ID!, title: String!, description: String, labels: [String!], category: String, sourceUrl: String, notes: String): Metadata!`: Add a metadata entry to a folder.
- `updateMetadata(id: ID!, folderId: ID!, title: String!, ...): Metadata!`: Update metadata details.
